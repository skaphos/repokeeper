...
var _ = Describe("Engine", func() {
	...
	It("syncs repositories with dry-run", func() {
		...
		eng := engine.New(&config.Config{Defaults: config.Defaults{TimeoutSeconds: 1, Concurrency: 1}}, reg, vcs.NewGitAdapter(nil), nil, nil, nil)
		...
		Expect(results[0].Outcome).To(Equal(engine.SyncOutcomePlannedFetch))
	})
	...
	It("filters dirty and clean during sync", func() {
		...
		eng := engine.New(&config.Config{Defaults: config.Defaults{TimeoutSeconds: 1, Concurrency: 2}}, reg, vcs.NewGitAdapter(runner), nil, nil, nil)
		results, err := eng.Sync(context.Background(), engine.SyncOptions{
	...
	It("filters diverged during sync", func() {
		runner := &mockRunner{responses: map[string]mockResponse{
			"/repo1:rev-parse --is-bare-repository":    {out: "false"},
	...
	It("respects concurrency by not exceeding it", func() {
		...
		eng := engine.New(&config.Config{Defaults: config.Defaults{TimeoutSeconds: 2, Concurrency: 1}}, reg, vcs.NewGitAdapter(blocker), nil, nil, nil)
	...
	It("times out long-running git operations", func() {
		...
		eng := engine.New(&config.Config{Defaults: config.Defaults{TimeoutSeconds: 1, Concurrency: 1}}, reg, vcs.NewGitAdapter(blocker), nil, nil, nil)
		results, err := eng.Sync(context.Background(), engine.SyncOptions{
			Concurrency: 1,
			Timeout:     1,
		})
	...
	It("produces accurate status json across many repos", func() {
		...
		eng := engine.New(&config.Config{Defaults: config.Defaults{TimeoutSeconds: 1, Concurrency: 4}}, reg, vcs.NewGitAdapter(&mockRunner{responses: responses}), nil, nil, nil)
	...
	It("classifies sync fetch failures", func() {
		...
		eng := engine.New(&config.Config{Defaults: config.Defaults{TimeoutSeconds: 1, Concurrency: 1}}, reg, vcs.NewGitAdapter(failing), nil, nil, nil)
	...
	It("pushes local commits when branch is ahead and push-local is enabled", func() {
		runner := &mockRunner{responses: map[string]mockResponse{
			"/repo1:-c fetch.recurseSubmodules=false fetch --all --prune --prune-tags --no-recurse-submodules": {out: ""},
			"/repo1:rev-parse --is-bare-repository":    {out: "false"},
			"/repo1:remote":                            {out: "origin"},
		...
		Expect(results[0].Outcome).To(Equal(engine.SyncOutcomePushed))
	})
