...
- [x] `go run -modfile=tools/go.mod github.com/golangci/golangci-lint/v2/cmd/golangci-lint run ./...` passes
- [x] `go run -modfile=tools/go.mod github.com/onsi/ginkgo/v2/ginkgo ./...` runs (even if no tests yet)
...
- [x] `go run -modfile=tools/go.mod github.com/golangci/golangci-lint/v2/cmd/golangci-lint run ./...` passes with zero issues
...
- [x] P1: Add benchmark tracking workflow (`go run -modfile=tools/go.mod github.com/go-task/task/v3/cmd/task perf-bench`) that saves raw benchmark logs and appends structured history (`perf/history.jsonl`) for trend analysis over time.
