...
func provisionWSL(ctx context.Context, cell Cell, prefix string) (ProvisionResult, error) {
	...
	script := fmt.Sprintf("set -eu; rm -rf /tmp/git-source /opt/repokeeper-git; mkdir -p /tmp/git-source /opt/repokeeper-git; tar -xJf %q -C /tmp/git-source --strip-components=1; cd /tmp/git-source; make configure; ./configure --prefix=/opt/repokeeper-git; make -j2 NO_CURL=YesPlease NO_EXPAT=YesPlease NO_GETTEXT=YesPlease all; make NO_CURL=YesPlease NO_EXPAT=YesPlease NO_GETTEXT=YesPlease install", linuxArchive)
