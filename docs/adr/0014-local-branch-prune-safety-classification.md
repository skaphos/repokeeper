...
A branch is **integrated** into base when `MergedIntoBase == true` (branch tip is an ancestor of base — reachability) **or** `PatchEquivalentToBase == true` (every branch commit is patch-equivalent to a commit already in base, via `git cherry`). Reachability backs `safe_to_prune`; patch-equivalence recognizes squash/rebase merges and backs only `probably_safe`. Because patch-equivalence is a **per-branch `git cherry`** (O(N) git calls per repo), it is computed **only when `require_merged` is disabled** — under the default `require_merged: true`, only reachability is trusted, so the engine skips patch-equivalence entirely and inspection stays at a single reachability query per repo. Base is resolved **per repo** and compared against its remote-tracking form (see [ADR-0015](0015-branch-retention-and-protection-policy.md)) so a stale local base cannot produce false "unmerged."
...
4. **`probably_safe`** — `PatchEquivalentToBase == true` but not reachability-merged (the squash/rebase-merge case), **only when `require_merged` is disabled**. Under the default `require_merged: true`, patch-equivalence is neither consulted nor computed, so a squash-merged branch is classified by reachability and upstream state like any other non-reachable branch (typically `needs_review`) and the conservative default **never emits `probably_safe`**.
...
- Real new surface area: a per-branch model, new gitx enumeration (including `%(worktreepath)`), and three brand-new git signals — reachability (`--merged`/`for-each-ref --merged=`), patch-equivalence (`git cherry`), and recency (`committerdate`) — none of which exist today. Patch-equivalence is an extra per-branch `git cherry`, so it runs only when `require_merged` is disabled; the default path issues a single reachability query per repo.
