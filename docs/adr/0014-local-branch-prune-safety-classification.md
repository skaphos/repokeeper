...
4. **`probably_safe`** — `PatchEquivalentToBase == true` but not reachability-merged (the squash/rebase-merge case). Because patch-equivalence is a heuristic, it is gated by policy: under `require_merged` (the default), such a branch is surfaced as `needs_review` with the `patch_equivalent_to_base` reason rather than `probably_safe`, so the conservative default **never emits `probably_safe`**. Only when `require_merged` is disabled does patch-equivalence yield `probably_safe` (`patch_equivalent_to_base`).
