...
- Adapter JSON uses a versioned object with named payloads; see the [adapter contract](docs/adapter-contract.md) for fields and migration notes. MCP config and registry resources use snake_case fields and omit unknown observation timestamps. `describe -o json` reads live metadata without updating the on-disk cache. Batch commands may return valid JSON alongside a nonzero exit when individual repositories fail.
